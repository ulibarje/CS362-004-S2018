...
int scoreFor (int player, struct gameState *state) {
  ...
  //score from deck 
  for (i = 0; i < state->deckCount[player]; i++)  // *Fixed bug* - state->discardCount to state->deckCount
    {
...
void adventurerCard(struct gameState *state, int currentPlayer, int* temphand, int handPos){
	...
	while(drawntreasure<2){
		...
		if (state->deckCount[currentPlayer] < 1){	// This if statement is pointless it shuffles nothing
			shuffle(currentPlayer, state);
		}
		drawCard(currentPlayer, state);
		
		//top card of hand is most recently drawn card.
		cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-2];	// *Added Bug* -changed from -1 to -2 so the last card drawn is ignored
	...
	//discard card
	// discardCard(handPos, currentPlayer, state, 0); // *Fixed Bug* Added this line to fix bug 
}
...
void stewardCard(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2, int choice3){
	...
	// *Added Bug* - changed from choice == 2 to choice == 1 so when 
	// this card is played it always add 2 cards and 2 coins
    else if (choice1 == 1){ 
...
// Each player discards the top card and gets a curse card on the top
// This card has a bug that needs to be fixed
void seahagCard(struct gameState *state, int currentPlayer){
...
void councilroomCard(struct gameState *state, int currentPlayer, int handPos){
	...
	for (i = 0; i < state->numPlayers; i++){
		if ( i != currentPlayer && i == currentPlayer){	// *Added Bug* - this makes no player draw a card
			drawCard(i, state);
		}
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		adventurerCard(state, currentPlayer, temphand, handPos);
		return 0;
