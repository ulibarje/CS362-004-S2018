...
int main() {
	...
	int i, randPlayer, randHandPos, test;
	...
	for(test = 0; test < iterations; test++) {
		...
		refGame.hand[randPlayer][randHandPos] = great_hall;
		...
		cardEffect(great_hall, choice1, choice2, choice3, &testGame, randHandPos, &bonus);
	...
	printf("\"numActions\" variable test 			---> %d/%d tests PASSED\n", (iterations - numActionsTC), iterations);
